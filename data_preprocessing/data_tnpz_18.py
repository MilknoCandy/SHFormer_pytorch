"""
❤Descripttion: (For ISIC2018 only) Convert the data file from jpg/png format to npz format for saving, 
while resizing (opencv), and use the saved data file for training (note: use the original image for testing)
❤version: 1.0
❤Author: MilknoCandy
❤Date: 2022-12-06 20:02:07
❤LastEditTime: 2023-12-16 20:42:18
...
if __name__ == '__main__':
    ...
    """ISIC2018: image is xx.jpg, segmentation is xx.png"""
    # all images are resized to [192, 256] and saved as .npz file
    H, W = 512, 512
    ...
    for tr in training_cases:
        case_id = tr[:-4].replace('_segmentation', '')

        # get path
        input_seg_file = join(lab_pth, tr)
        input_img_file = join(img_pth, case_id+'.jpg')
