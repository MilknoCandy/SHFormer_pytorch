"""
❤Descripttion: trainer for polyp dataset
❤version: 1.0
❤Author: MilknoCandy
❤Date: 2022-11-30 16:14:27
❤LastEditTime: 2024-03-22 15:03:26
...
from training.data_loading.dataset_loading import (CustomDataSet,
                                                   MulTestDataset, TestDataSet,
                                                   load_dataset)
...
from utils.metrics.compute_metrics import AllMterics, AverageMeter
from utils.tensor_utilities import sum_tensor
from utils.to_torch import maybe_to_torch, to_cuda


class SHFormerTrainer_polyp(NetworkTrainer):
    def __init__(
        ...
        self.ts_mul = False     # 表示测试用数据集是否为多个
    ...
    def load_dataset(self):
        self.dataset, self.ts_mul = load_dataset(self.dataset_directory, data_type=self.data_type, data_suffix="npz")
    ...
    def get_basic_generators(self):
        ...
        if not self.ts_use_npz:
            if not self.ts_mul:
                ...
                # 多个测试集，因此多个迭代器
                val_dl = OrderedDict()
                for img_pth, lab_pth in zip(self.ts_img_pth, self.ts_lab_pth):
                    ts_name = img_pth.rsplit('/', 2)[-2]
                    v_gen = MulTestDataset(
                        img_pth=img_pth, lab_pth=lab_pth, 
                        transform_list=self.cfg.Val_transform_list, one_Hot=self.mask_onehot
                    )
                    v_dl = DataLoader(v_gen, batch_size=1, shuffle=False, num_workers=self.cfg.dataset.num_workers, pin_memory=self.pin_memory)
                    val_dl[ts_name] = v_dl
    ...
    def validate(self, test_output_folder: str="Test_result", gt_oneHot=False):
        ...
        if not self.ts_use_npz:
            if not self.ts_mul:
                ...
                for img_pth, lab_pth in zip(self.ts_img_pth, self.ts_lab_pth):
                    ts_name = img_pth.rsplit('/', 2)[-2]
                    tss_gen = MulTestDataset(
                        img_pth=img_pth, lab_pth=lab_pth, 
                        transform_list=self.cfg.Val_transform_list, one_Hot=self.mask_onehot
                    )
                    t_dl = DataLoader(tss_gen, batch_size=1, shuffle=False, num_workers=self.cfg.dataset.num_workers, pin_memory=self.pin_memory)
                    ts_dl[ts_name] = t_dl
        else:
            ts_gen = CustomDataSet(self.dataset_ts, transform_list=self.cfg.Test_transform_list, oneHot=gt_oneHot)
            ts_dl = DataLoader(ts_gen, batch_size=1, shuffle=False, num_workers=self.cfg.dataset.num_workers, pin_memory=self.pin_memory)

        all_datasets_metrics = OrderedDict()
        # logger.info("\n=============================Testing=============================")
        ...
        with alive_bar(len(ts_dl), title="Testing", bar='halloween', spinner='loving', length=80, force_tty=True) as bar:
            for datas_num, datas in ts_dl.items():
                ...
                for i, data in enumerate(datas):
                    ...
                    all_datasets_metrics[datas_num] = all_metrics
                    
                    bar()
                

            table_header = ['Dataset', 'Model_name', 'mDice', 'mIoU', 'mAcc', 'TJI', 'sen', 'spe', 'precision']
            table_data = []
            sum_metrics = torch.zeros(8)      # global average
            avg_metrics = torch.zeros_like(sum_metrics)
            for n, mt in all_datasets_metrics.items():
                full_metrics = torch.tensor([
                    [mt.dice.avg, mt.jc.avg, mt.acc.avg, mt.tji.avg, mt.sen.avg, mt.spe.avg, mt.prec.avg, 1],
                    [mt.dice.sum, mt.jc.sum, mt.acc.sum, mt.tji.sum, mt.sen.sum, mt.spe.sum, mt.prec.sum, mt.dice.count]])
                
                table_data.append((
                    n, self.cfg.model.model_name,
                    full_metrics[0][0], # mt.dice.avg, 
                    full_metrics[0][1], # mt.jc.avg, 
                    full_metrics[0][2], # mt.acc.avg, 
                    full_metrics[0][3], # mt.tji.avg,
                    full_metrics[0][4], # mt.sen.avg,
                    full_metrics[0][5], # mt.spe.avg,
                    full_metrics[0][6], # mt.prec.avg,
                ))
                sum_metrics += full_metrics[0, :]
                avg_metrics += full_metrics[1, :]
            sum_metrics = sum_metrics / sum_metrics[-1]
            avg_metrics = avg_metrics / avg_metrics[-1]
            table_data.append((
                "dataset_average", self.cfg.model.model_name,
                sum_metrics[0], sum_metrics[1], sum_metrics[2], sum_metrics[3],
                sum_metrics[4], sum_metrics[5], sum_metrics[6],
            ))
            table_data.append((
                "global_average", self.cfg.model.model_name,
                avg_metrics[0], avg_metrics[1], avg_metrics[2], avg_metrics[3],
                avg_metrics[4], avg_metrics[5], avg_metrics[6],
            ))
