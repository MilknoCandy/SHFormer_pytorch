...
class SHFormerTrainer(NetworkTrainer):
    def __init__(
        self, cfg=None, data_type='0', output_folder=None, dataset_directory=None, sed=0, deterministic=True, fp16=False, fold='all'
    ):
    ...
    def output_seg(self, save_output_folder: str="Test_result", gt_oneHot=False):
        ...
        # ind_select = [47, 63, 175, 192, 193, 194, 334]
        ind_select = [47, 63, 194]
        ind_select = set(ind_select)   
        ...
        with alive_bar(len(ts_dl), title="Testing", bar='halloween', spinner='loving', length=80, force_tty=True) as bar:
            for i, data in enumerate(ts_dl):
                ...
                imageio.imwrite(save_here, pred_mask)
                bar()
    ...
    def eval_speed(self, input_size=(1, 3, 512, 512), iters=100):
        print("=============================Testing Speed=============================")
        compute_speed(self.network, input_size=input_size, iteration=iters)