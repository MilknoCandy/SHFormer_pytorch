...
❤LastEditTime: 2023-12-16 21:07:44
...
from ..shformer.decode_head_type import decode_head_sim, decode_head_best, decode_head_sand
from network_architecture.network_tools.initilization import  InitWeights_He


class shformer_add_dy(nn.Module):
    def __init__(self, in_chans=3, num_classes=1, embed_dims=[32, 64, 160, 256], heads=[1, 2, 5, 8], linear_bias=True, mlp_ratio=4.,
        ...
        self.decode_heade = decode_head_sand(embed_dims=embed_dims, norm_layer=norm_layer, num_classes=num_classes)
    ...
    def forward(self, img):
        ...
        #           ENCODER
        ###################################################
        feats, mask_tokens, mask_prob_tokens = self.backbone(img)
        ###################################################
        #           DECODER
        ###################################################
