...
❤LastEditTime: 2023-12-16 21:09:54
...
from ..decode_head_type import decode_head_MLA
from network_architecture.network_tools.initilization import  InitWeights_He


class shformer_add_MLA(nn.Module):
    def __init__(self, in_chans=3, num_classes=1, embed_dims=[32, 64, 160, 256], heads=[1, 2, 5, 8], linear_bias=True, mlp_ratio=4.,
        ...
        #               Decoder
        ###################################################
        self.decode_heade = decode_head_MLA(dims=embed_dims, dim=128, class_num=num_classes)
    ...
    def forward(self, img):
        ...
        pred = resize(pred, size=img.shape[2:], mode='bilinear', align_corners=False)
        return pred