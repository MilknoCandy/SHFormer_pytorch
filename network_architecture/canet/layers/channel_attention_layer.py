...
class SE_Conv_Block(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_out=False):
        ...
        if planes <= 16:
            self.globalAvgPool = nn.AvgPool2d((512, 512), stride=1)  # (224, 300) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((512, 512), stride=1)
        elif planes == 32:
            self.globalAvgPool = nn.AvgPool2d((256, 256), stride=1)  # (112, 150) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((256, 256), stride=1)
        elif planes == 64:
            self.globalAvgPool = nn.AvgPool2d((128, 128), stride=1)    # (56, 75) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((128, 128), stride=1)
        elif planes == 128:
            self.globalAvgPool = nn.AvgPool2d((64, 64), stride=1)    # (28, 37) for ISIC2018
